...
$app.rootCmd.addCommand(
	new Command({
		...
		run: (cmd, args) => {
			// 确保 app.os 被正确初始化
			if (!app.os || typeof app.os.readFile !== 'function') {
				console.error('app.os is not initialized or readFile is not a function');
				return;
			}
			if (args.length < 2) {
			...
			const file = String.fromCharCode.apply(null, app.os.readFile(args[0]))
			const config = {
				dynamicTyping: true,
				header: true,
			}
			...
			app.RunInTransaction((txApp) => {
				for (let i = 0; i < r1.data.length; i++) {
					if (Object.keys(r1.data[i]).length === columns.length) {
						txApp.Save(new Record(collection, r1.data[i]))
